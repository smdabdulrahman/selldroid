...
class StockSaleReportScreen extends StatefulWidget {
  ...
  @override
  State<StockSaleReportScreen> createState() => _StockSaleReportScreenState();
}
...
class _StockSaleReportScreenState extends State<StockSaleReportScreen> {
  ...
  Future<void> _loadData() async {
    ...
    // 2. Fetch Totals (Added: is_stock_sales = 1)
    final revenueRes = await db.rawQuery(
      '''
      SELECT SUM(final_amount) as total 
      FROM sales 
      WHERE billed_date >= ? AND billed_date <= ? AND is_stock_sales = 1
    ''',
      [startStr, endStr],
    );
    ...
    final itemsRes = await db.rawQuery(
      ...
      WHERE s.billed_date >= ? AND s.billed_date <= ? AND s.is_stock_sales = 1
    ''',
      [startStr, endStr],
    );
    ...
    // 3. Fetch Chart Data (Added: is_stock_sales = 1)
    ...
    // Process chart data with custom date support
    _processChartData(
      chartRes,
      startDate,
    ...
    // 4. Fetch Top Selling Items (Added: is_stock_sales = 1)
    ...
    // 5. Fetch Recent Transactions (Added: is_stock_sales = 1, Limit 5)
