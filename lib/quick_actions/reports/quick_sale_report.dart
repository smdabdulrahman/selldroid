...
class QuickSaleReportScreen extends StatefulWidget {
  ...
  @override
  State<QuickSaleReportScreen> createState() => _QuickSaleReportScreenState();
}
...
class _QuickSaleReportScreenState extends State<QuickSaleReportScreen> {
  ...
  Future<void> _loadData() async {
    ...
    // 2. Fetch Totals (Added: is_stock_sales = 0)
    final revenueRes = await db.rawQuery(
      '''
      SELECT SUM(final_amount) as total 
      FROM sales 
      WHERE billed_date >= ? AND billed_date <= ? AND is_stock_sales = 0
    ''',
      [startStr, endStr],
    );
    ...
    final itemsRes = await db.rawQuery(
      ...
      WHERE s.billed_date >= ? AND s.billed_date <= ? AND s.is_stock_sales = 0
    ''',
      [startStr, endStr],
    );
    ...
    // 3. Fetch Chart Data (Added: is_stock_sales = 0)
    ...
    // 4. Fetch Top Selling Items (Added: is_stock_sales = 0)
    ...
    // 5. Fetch Recent Transactions (Added: is_stock_sales = 0, Limit 5)
